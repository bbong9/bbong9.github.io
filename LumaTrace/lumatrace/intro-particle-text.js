(() => {
  const INTRO_SESSION_KEY = 'lumatrace-wiki-intro-entered-at';
  ...
  const getWords = (canvas) => (canvas.dataset.words || 'WELCOME TO|LUMATRACE|DESIGN BY|BBONG9')
    .split('|')
  ...
  const startIntro = (section) => {
    ...
    const startHomeReveal = () => {
      window.requestAnimationFrame(() => {
        ...
        window.setTimeout(() => {
          window.dispatchEvent(new CustomEvent('lumatrace:home-reveal'));
        }, 520);
      });
    };
